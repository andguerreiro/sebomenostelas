#!/bin/sh
set -eu
# V4 do Sebo Menos Telas.
# Uso: ./gerar_site.sh [catalogo.csv] [site] [index.html] [sobre.html] [favicon.ico]
...
python3 - "$CSV" "$OUT" "$INDEX_TEMPLATE" "$SOBRE_TEMPLATE" "$FAVICON_TEMPLATE" "$BASE_URL" "$URLMAP" <<'PY'
...
urlmap_path = Path(sys.argv[7]).expanduser().resolve()
...
search_data = []
for r in rows:
    search_data.append({"isbn": r["ISBN"], "autor": r["Autor"], "titulo": r["Titulo"], "editora": r["Editora"], "ano": r["Ano"], "estante": r["Estante"], "preco": r["Preco"], "peso": r["Peso"], "idioma": r["Idioma"], "slug": r["_slug"]})
...
print(f"Favicon: {'Copiado (' + str(favicon_template) + ')' if favicon_copied else 'Não encontrado (ignorado)'}")
