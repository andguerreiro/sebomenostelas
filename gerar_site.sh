...
if not index_template.exists():
    raise SystemExit(f"Template não encontrado: {index_template}")
...
required = [
    "ISBN", "Autor", "Titulo", "Editora", "Ano", "Estante",
    "Preco", "Peso", "Idioma", "Capa", "Paginas", "Dimensoes"
]
...
def norm(v):
    return " ".join(
        "".join(
            c for c in unicodedata.normalize("NFKD", str(v or ""))
...
def make_slug(r, key):
    ...
    if isbn and isbn.lower() not in {"nd", "n/d", "na", "n/a", "sem isbn"}:
        parts.append(slug_text(isbn))
...
if urlmap_path.exists():
    try:
        urlmap = json.loads(urlmap_path.read_text(encoding="utf-8"))
    except Exception as e:
        raise SystemExit(f"Não foi possível ler {urlmap_path.name}: {e}")
else:
    urlmap = {}

if not isinstance(urlmap, dict):
    raise SystemExit(f"{urlmap_path.name} precisa conter um objeto JSON.")
...
urlmap_path.write_text(
    json.dumps(urlmap, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
    encoding="utf-8"
)

# Todos estes campos ficam disponíveis para a busca no index.html.
search_data = []
...
(out / "dados" / "catalogo.json").write_text(
...
for r in rows:
    wa_text = (
        f"Olá! Gostaria de receber fotos detalhadas deste exemplar:\n\n"
        f"{r['Titulo']}, {r['Autor']} — {price(r['Preco'])}"
    )
...
<tr><th>Título</th><td>{esc(r["Titulo"])}</td></tr><tr><th>Autor</th><td>{esc(r["Autor"])}</td></tr><tr><th>Editora</th><td>{esc(r["Editora"])}</td></tr><tr><th>Ano</th><td>{esc(r["Ano"])}</td></tr><tr><th>ISBN</th><td>{esc(r["ISBN"])}</td></tr><tr><th>Estante</th><td>{esc(r["Estante"])}</td></tr><tr><th>Idioma</th><td>{esc(r["Idioma"])}</td></tr><tr><th>Capa</th><td>{esc(r["Capa"])}</td></tr><tr><th>Páginas</th><td>{esc(r["Paginas"])}</td></tr><tr><th>Dimensões</th><td>{esc(r["Dimensoes"])}</td></tr><tr><th>Peso</th><td>{esc(r["Peso"])} g</td></tr>
    ...
    (out / "livro" / f'{r["_slug"]}.html').write_text(
        page,
        encoding="utf-8"
    )
...
print(
    f"Logo: "
    f"{'Copiado (' + str(logo_template) + ')' if logo_copied else 'Não encontrado (ignorado)'}"
)
